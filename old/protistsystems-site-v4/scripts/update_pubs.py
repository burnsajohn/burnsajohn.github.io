...
def crossref_by_title(title, first_author=""):
    ...
    for item in items:
        ...
        # Exact normalized match, or one contained in the other, which
        # catches Scholar's truncated titles.
...
def main():
    ...
    for i, e in enumerate(source, 1):
        ...
        image = by_doi.get(doi) or by_title.get(normalize_title(title))
    ...
    lines = [
        "# Generated by scripts/update_pubs.py. Do not edit by hand except to",
        "# set `image:` values, which are preserved when the script re-runs.",
        "#",
        "# Cover images go in images/pubs/ using the slug suggested above each",
        "# entry, then uncomment that entry's `image:` line.",
        "",
    ]
    for e in entries:
        ...
        if e["image"]:
            ...
            lines.append(f"  # image: images/pubs/{e['slug']}.jpg")
        if e["preprint"]:
    ...
    missing = sum(1 for e in entries if not e["image"])
    if missing:
        print(f"\n{missing} entries have no cover image and will use the "
              f"placeholder.")
